// ============================================================
// assessment.js — PHS Workshop Processes (single-file logic)
...
function buildWorkshopProcessRecordFromResults(results = [], assessment = null) {
  const answerFor = (id) => {
  ...
  return {
    projectName: assessment?.projectName || answerFor("rs_project_name"),
    materials: answerFor("rs_materials_used"),
    markOut: answerFor("rs_mark_out"),
    cutShape: answerFor("rs_cut_shape"),
    joints: answerFor("rs_joints_jointing"),
    assemble: answerFor("rs_assemble"),
    finishes: answerFor("rs_finishes"),
  };
}
...
function drawTextInPdfBox(page, text, box, font, options = {}) {
  ...
  const maxSize = options.maxSize ?? 8.5;
...
async function stampWorkshopProcessPdf(doc, finalData) {
  ...
  const page2 = pages[1];
  const page3 = pages[2];
  const record = finalData.processRecord || {};
  ...
  drawSingleLinePdfText(page3, finalData.teacherName, {
    x: 109,
    y: 322.0,
    width: 484,
    height: 16,
  ...
  drawSingleLinePdfText(page3, new Date().toLocaleDateString("en-NZ"), {
    x: 511,
    y: 303.0,
    width: 80,
    height: 16,
...
async function fillPdfForm(pdfBytes, finalData) {
  ...
  // Backward compatibility with the previous one-page fillable sign-off sheet.
  ...
  // The replacement 3-page Workshop Processes sheet is non-fillable.
  if (finalData.processRecord?.projectName) {
    await stampWorkshopProcessPdf(doc, finalData);
  }
...
function loadAssessment() {
  ...
  ass.questions.forEach((q) => {
    ...
    if (q.readOnly && q.prefill) field.value = q.prefill;
    else if (prev) field.value = prev;
    else if (q.prefill) field.value = q.prefill;
...
function submitWork() {
  ...
  finalData = {
    ...
    attachSignoff: !!selectedAssessment.attachSignoff,
    processRecord: buildWorkshopProcessRecordFromResults(results, selectedAssessment),
    points: total,
    totalPoints,
    pct,
